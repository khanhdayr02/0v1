...
const langData = {
    "vi_VN": {
        ...
        "success": "Thích Thì Tao Bậy Lên Đó",
    },
    "en_US": {
...
async function confirmChange({ message, getLang, data, eventData }) {
    ...
    if (!newSettings || !data?.thread?.info) return message.send(getLang("error"));

    try {
        await global.controllers.Threads.updateData(message.threadID, { antiSettings: newSettings });
...
async function chooseMenu({ message, getLang, data }) {
    try {
        let chosenIndexes = message.args.filter(e => !!e && !isNaN(e) && e > 0 && e <= 6);
        ...
        const _newMessage = await message.reply(getLang("newSettings", { ..._newSettings }));
        _newMessage.addReactEvent({ callback: confirmChange, newSettings, participantIDs: message.participantIDs });
